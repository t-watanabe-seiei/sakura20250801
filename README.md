...
### むずかしいレベル（`/advanced`）
- 10～18の数から適切な数をひく減法（答えは1～9の1桁）
- 例：15 - 7 = 8、18 - 9 = 9、12 - 5 = 7 など
- キーボード入力に対応（1桁の数字）
...
### 効果音（強化版）
- Web Audio APIを使用した和音効果音が再生されます
- **通常正解**：3段階の和音（C+E → E+G → G）で華やかに
- **5問連続正解**：4段階の和音（C+E → E+G → G+C(高) → C+E+G）でさらに豪華に
- 音量も増幅され、より聞き取りやすく

## キーボード入力機能
- **キーボードでの回答が可能**
  - 画面に表示されている数字キー（0〜9）を押すだけで回答できます
  - ボタンをクリックしなくても素早く回答可能
  - 通常レベル、むずかしいレベル両方で利用可能

## むずかしいレベルの選択肢
- **正解の前後の数字**を選択肢として表示（0～9の範囲）
  - 正解の±1、±2、±3の範囲から選択肢を生成
  - 1桁の数字のみ表示（キーボード入力対応）
  - より考えさせる良問に

## 更新履歴
### 2026-01-17 (3回目の更新)
- **[重要な修正]** むずかしいレベルの答えを1桁（1～9）に制限
  - キーボード入力に完全対応
  - 問題生成ロジックを改善
  - 選択肢も0～9の範囲に調整

### 2026-01-17 (2回目の更新)
- **[機能追加]** キーボード入力での回答機能を追加（両レベル対応）
- **[改善]** 効果音を和音化し、音量を増幅（より豪華で聞き取りやすく）
- **[改善]** むずかしいレベルの選択肢を正解の前後の数字に変更（負の数を回避）
